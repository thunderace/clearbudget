...
function transactionEditForm(formId, loadingImage, id) {
  ...
  $.ajax({
    ...
    success: function(data, status){
      ...
      else $('#'+loadingImage).attr({src: "style/icons/accept.png"});
      showStatus(data.msg, 3000);
...
function setReport(id) {
  ...
  if(!id) {
    id = $("#reportSelector").val();
  }
  // show the loading div
  showLoading(true);
  $.ajax({
    type: "POST", timeout: 15000, url: "index.php", dataType: 'json',cache: false,
    data: "ajax=1&action=setReport&reportId="+id,
    success: function(data, status){
